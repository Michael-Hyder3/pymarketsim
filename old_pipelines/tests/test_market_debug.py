"""
Test the market order matching with simple buyers and sellers - detailed debugging.
"""
...
print("=" * 80)
print("TESTING MARKET ORDER MATCHING - DETAILED DEBUG")
print("=" * 80)
...
for buyer in buyers:
    ...
    if orders:
        ...
        order = orders[0]
        print(f"Buyer {buyer.get_id()} placed bid: {order.price:.2f} (type={order.order_type}, qty={order.quantity})")
    else:
...
for seller in sellers:
    ...
    if orders:
        ...
        order = orders[0]
        print(f"Seller {seller.get_id()} placed ask: {order.price:.2f} (type={order.order_type}, qty={order.quantity})")
    else:
...
# Show event queue before adding
print(f"\nEvent queue before adding orders:")
print(f"  Current time: {market.get_time()}")

# Add orders one by one
for order in all_orders:
    market.add_orders([order])
    print(f"  Added: {'BUY' if order.order_type == BUY else 'SELL'} {order.price:.2f}")

print(f"\nEvent queue after adding {len(all_orders)} orders:")
print(f"  Current time: {market.get_time()}")
print(f"  Queue size: {market.event_queue}")
...
if matched_orders:
    for mo in matched_orders:
        print(f"  Matched: {'BUY' if mo.order.order_type == BUY else 'SELL'} at {mo.order.price:.2f} by agent {mo.order.agent_id}")
else:
    print("  NO MATCHES!")
    
print(f"\nOrder book state after step:")
print(f"  Best bid: {market.order_book.get_max_bid()}")
print(f"  Best ask: {market.order_book.get_min_ask()}")
