...
class SimulatorSampledArrivalCustom:
    """A copy of the sampled-arrival simulator that optionally creates an HBL agent
    alongside the background ZI agents. This class is added without modifying the
    original simulator implementation so experiments can be run in parallel.
    """
    def __init__(self,
        ...
        # Create background ZI agents and optionally an HBL agent without removing
        # the original simulator. Agent IDs are assigned consecutively.
        next_agent_id = 0
        ...
        # If requested, add one HBL agent (with the next id)
        if self.hbl_agent:
            hbl_id = next_agent_id
            # schedule the HBL agent's first arrival
            self.arrivals[self.arrival_times[self.arrival_index].item()].append(hbl_id)
            self.arrival_index += 1
            self.agents[hbl_id] = HBLAgent(
                agent_id=hbl_id,
                market=self.markets[0],
                q_max=q_max,
                shade=shade,
        ...
        if self.time < self.sim_time:
            for market in self.markets:
                ...
                for agent_id in agents:
                    ...
                    # Some agents (HBLAgent) require a `side` argument while ZIAgent does not.
                    # Call without args first; if that fails, call with a random side.
                    try:
                        orders = agent.take_action()
                    except TypeError:
                        # fallback to passing side argument
